...
load_temp<- data.frame(load_processed$Date, load_processed$demand_daily, temperature_processed$`mean(hourly_mean_temp)`)


#NN_fit <- nnetar(ts_act_power_daily_train,p=1,P=1)
NN_fit6 <- nnetar(ts_daily_demand,p=1,P=0,xreg=fourier(ts_daily_demand))
...
#NN_fit <- nnetar(ts_act_power_daily_train,p=1,P=1)
NN_fit8 <- nnetar(ts_daily_demand,p=1,P=0,xreg=fourier(ts_daily_demand2, K=c(1,1)))

#NN_for <- forecast(NN_fit, h=365) 
NN_for8 <- forecast(NN_fit8, h=59,xreg=fourier(ts_daily_demand2, 
                                          K=c(1,1),h=59))
...
#NN_fit <- nnetar(ts_act_power_daily_train,p=1,P=1)
NN_fit10 <- nnetar(ts_daily_demand,p=,P=1,xreg=fourier(ts_daily_demand, K=c(1,1)))
...
#NN_fit <- nnetar(ts_act_power_daily_train,p=1,P=1)
NN_fit12 <- nnetar(ts_hourly_demand,p=1,P=1,xreg=fourier(ts_hourly_demand, K=c(2,2,2)))

#NN_for <- forecast(NN_fit, h=365) 
NN_for12 <- forecast(NN_fit12, h=59,xreg=fourier(ts_hourly_demand, 
                                          K=c(2,2,2),h=59))
...
```{r}
janfeb14<- janfeb%>%
  mutate(load=as.matrix(NN_for12$mean))%>%
  select(date, load)

write.csv(janfeb13, file = "./Competition/Output/model18.csv", row.names = FALSE)
```
